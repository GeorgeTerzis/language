...
struct ref {
    ...
    inline pointer checked_ptr(const std::source_location loc =
    ...
    inline reference
    deref(std::source_location loc = std::source_location::current()) {
    ...
    void* as_void() const { return data; }
    uintptr_t as_uint() const { return reinterpret_cast<uintptr_t>(data); }
    ...
    cpointer ptr() const { return data; }
...
struct strong_ref {
    ...
    cpointer operator->() const { return data; }

    creference
    get(std::source_location loc = std::source_location::current()) const {
        if (!data)
            throw null_init_error(loc);
        return *data;
    }
    ...
    void replace(T& other) { data = &other; }

    pointer ptr() { return data; }
    const pointer ptr() const { return data; }

    reference deref() { return *data; }
    creference deref() const { return *data; }
    void* as_void() const { return data; }
    uintptr_t as_uint() const { return reinterpret_cast<uintptr_t>(data); }
    ref<T> as_ref() const { return ref<T>{data}; }
