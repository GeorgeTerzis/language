...
struct string_map {
    ...
    // element access
    Val& operator[](llvm::StringRef k) { return data[k]; }
    ...
    size_type erase(llvm::StringRef key) { return data.erase(key); }

    void swap(string_map& other) noexcept { data.swap(other.data); }

    // lookup
    size_type count(llvm::StringRef key) const { return data.count(key); }

    iterator find(llvm::StringRef key) { return data.find(key); }

    const_iterator find(llvm::StringRef key) const { return data.find(key); }

    bool contains(llvm::StringRef key) const { return data.contains(key); }
};
...
struct map {
    ...
    // element access
    Val& operator[](const Key& k) { return data[k]; }
    Val& operator[](Key&& k) { return data[std::move(k)]; }
    Val& at(const Key& k) { return data.at(k); }
    const Val& at(const Key& k) const { return data.at(k); }
